...
class ATCNet(EEGModuleMixin, nn.Module):
    ...
    def forward(self, X):
        ...
        X = self.ensuredims(X)
        # print("Shape after Ensure4d:", X.shape)
        ...
        # print("Shape after ConvBlock:", conv_feat.shape)
        # Dimension: (batch_size, F2, Tc, 1)
        conv_feat = conv_feat.view(-1, self.F2, self.Tc)
        # print("Shape after view:", conv_feat.shape)
        ...
        for w in range(self.n_windows):
            ...
            # print(f"Shape after sliding window {w}:", conv_feat_w.shape)
            # Dimension: (batch_size, F2, Tw)

            # ----- Attention block -----
            att_feat = self.attention_blocks[w](conv_feat_w)
            # print(f"Shape after AttentionBlock {w}:", att_feat.shape)
            # Dimension: (batch_size, F2, Tw)

            # ----- Temporal convolutional network (TCN) -----
            tcn_feat = self.temporal_conv_nets[w](att_feat)[..., -1]
            # print(f"Shape after TCN {w}:", tcn_feat.shape)
...
class _ConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            ...
            padding="same",
            # padding = (kernel_length_1 // 2, 0),  # Manually calculate padding
            bias=False,
        )
        ...
        self.conv2 = nn.Conv2d(
            ...
            padding="valid",
            # padding=(0, 0),  # No padding needed here
            bias=False,
        )
        ...
        self.conv3 = nn.Conv2d(
            ...
            padding="same",
            # padding=(kernel_length_2 // 2, 0),  # Manually calculate padding
            bias=False,
        )
